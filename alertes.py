...
#  Chargement des variables d'environnement (EMAIL_SENDER, EMAIL_PASSWORD, EMAIL_DESTINATAIRE)
load_dotenv()
EMAIL_SENDER = os.getenv("EMAIL_SENDER")
EMAIL_PASSWORD = os.getenv("EMAIL_PASSWORD")
EMAIL_DESTINATAIRE = os.getenv("EMAIL_DESTINATAIRE")

# Lecture du fichier CSV contenant les données consolidées
def charger_donnees(path="cve_consolidated.csv"):
    return pd.read_csv(path)

# Filtrage des CVE critiques (CVSS ≥ 9 et "critical")
...
#  Formatage d’un message texte à envoyer
...
def main():
    ...
    #  Sauvegarde de toutes les alertes critiques détectées dans un fichier CSV
